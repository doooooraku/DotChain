...
export interface Settings {
  ...
  heatmapRangeDays: 30 | 60 | 180 | 365;
}
...
import { create } from "zustand";
import { Settings, LanguageCode, ThemeKey } from "@/types/models";
...
interface SettingsState {
  ...
  toggleEffect: () => Promise<void>;
  setHeatmapRange: (days: Settings["heatmapRangeDays"]) => Promise<void>;
}
...
export const useSettingsStore = create<SettingsState>((set, get) => ({
  ...
  async load() {
    ...
    set({ settings: loaded ?? defaultSettings, isReady: true });
  },
  ...
  async toggleSound() {
    ...
    const next = { ...cur, soundEnabled: !cur.soundEnabled };
    set({ settings: next });
    await settingsRepository.save(next);
  },

  // haptics, effect, heatmapRange も同様のパターン

  async setHeatmapRange(days) {
    const next = { ...get().settings, heatmapRangeDays: days };
    set({ settings: next });
    await settingsRepository.save(next);
  },
}));
