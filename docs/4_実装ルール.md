...
- 固定文言（ラベル・ヘルプテキスト・エラーメッセージなど）は必ず i18n キーで管理し、18言語ぶん翻訳する。
- ユーザーが入力する文字列（習慣名・メモ等）は i18n の対象外とし、そのまま表示する。
- 再利用コンポーネントは、原則「翻訳済み or ユーザー入力済みの文字列」を props で受け取り、内部では固定文言（アクセシビリティ補足など）だけ `t()` を使う。
